<template>
  <div class="form-group">
    ...
    <div v-for="(item, idx) in items" :key="idx" class="flex items-center gap-2 mb-2">
      ...
      <button
        type="button"
        class="shrink-0 p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
        title="Remove"
        @click="remove(idx)"
      >
...
function sync() {
  const trimmed = items.value.map((s) => s.trim()).filter(Boolean)
  emit('update:modelValue', trimmed.length ? items.value : null)
}
