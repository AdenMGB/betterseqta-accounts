...
const SIGNUP_ORDER_SQL_VARIANTS = [
  `SELECT id FROM users ORDER BY createdAt ASC, id ASC`,
  `SELECT id FROM users ORDER BY created_at ASC, id ASC`,
] as const;

const RECOMPUTE_SQL_VARIANTS = [
...
async function listUsersInSignupOrder(db: Env["DB"]): Promise<{ id: string }[]> {
  for (const sql of SIGNUP_ORDER_SQL_VARIANTS) {
    try {
      const result = await db.prepare(sql).all();
      return (result.results ?? []) as { id: string }[];
    } catch {
      // try next column name
    }
  }
  const result = await db.prepare(`SELECT id FROM users ORDER BY id ASC`).all();
  return (result.results ?? []) as { id: string }[];
}
...
export async function backfillSignupNumbers(db: Env["DB"]): Promise<number> {
  const rows = await listUsersInSignupOrder(db);
  let assigned = 0;
...
export async function recomputeAllSignupNumbers(db: Env["DB"]): Promise<number> {
  for (const sql of RECOMPUTE_SQL_VARIANTS) {
    try {
