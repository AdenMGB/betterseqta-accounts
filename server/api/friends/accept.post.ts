...
export default defineEventHandler(async (event: H3Event) => {
  ...
  if (!friendship || friendship.addresseeId !== decoded.id) {
    return sendError(event, createError({ statusCode: 404, statusMessage: 'Friend request not found or you are not the recipient' }))
  }
